...
下表のとおり、コードで対処可能な指摘はすべて反映済み。検証は `npm test`（28/28 合格）・`npm run typecheck`（クリーン）・`wrangler deploy --dry-run`（レート制限バインディングの配線を確認）。M2 のみ Cloudflare 管理画面側の運用設定が本体のため、コードでは二次防御に留め、手順を「[本番デプロイ時のハードニング（M2）](#本番デプロイ時のハードニングm2運用)」に記載した。
